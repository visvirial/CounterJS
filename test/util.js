...
function testTx(properties) {
	...
	var dest = properties.dest || 'msTBjkycK1ZmPq1EBkQUwvSYq2fm5KrpJJ';
	var message = properties.message || Message.createSend(util.assetNameToId('VISVIRIAL'), Long.fromString('100000000', true));
	var change = properties.change || {
		address: 'mtGffL93zFs3gdhcFo5DGkCQxSJFfdttYa',
		value: 99925290
	};
	return util.buildTransaction(inputs, dest, message, change, 'testnet');
}
...
describe('util', function() {
	...
	it('should calculate the change correctly', function () {
		var rawtx = testTx({
			change: {
				address: 'mtGffL93zFs3gdhcFo5DGkCQxSJFfdttYa',
				fee_per_kb: 78125
		...
		assert(tx.outs[tx.outs.length - 1].value == 99933329);
	});
	it('should reduce the change dest value to accomodate fee', function () {
		var rawtx = testTx({
			dest: {
		...
		var tx = bitcoin.Transaction.fromBuffer(rawtx);
		assert(tx.outs[0].value == 99938759);
	});
	it('should not reduce the dest output, but throw an error', function () {
